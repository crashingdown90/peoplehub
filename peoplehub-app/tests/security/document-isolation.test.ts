...
describe("Security: Document Service Tenant Isolation", () => {
    ...
    describe("Employee Role-Based Isolation", () => {
        it("should limit employee to only their own documents", async () => {
            ...
            (prismaMock.auditLog.findMany as jest.Mock).mockResolvedValue(mockDocuments);
            (prismaMock.auditLog.findFirst as jest.Mock).mockResolvedValue(null);

            const result = await DocumentService.getDocuments(contextTenantA_Employee, {
                page: 1,
                limit: 10,
            });
            ...
            if (result.success && result.data) {
                result.data.forEach((doc) => {
                    expect(doc.employeeId).toBe("emp-a");
                });
            }
        });
    });
