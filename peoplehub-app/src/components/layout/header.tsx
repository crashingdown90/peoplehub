    ...
    return (
        <header className="sticky top-0 z-40 flex items-center justify-between border-b border-(--color-border) bg-(--color-surface)/90 px-4 py-3 backdrop-blur transition-colors">
            <div className="flex items-center gap-3">
                ...
                <Link href="/dashboard" className="text-lg font-semibold text-(--color-accent)">
                    PeopleHub
                </Link>
                {showSearch && (
                    <div className="hidden w-72 items-center gap-2 rounded-lg border border-(--color-border) bg-(--color-surface) px-3 py-2 text-sm text-(--color-text-subtle) shadow-sm transition hover:border-(--color-accent) focus-within:border-(--color-accent) focus-within:ring-2 focus-within:ring-(--color-accent)/20 md:flex">
            ...
            <div className="flex items-center gap-3">
                ...
                <Link
                    href="/settings"
                    className="hidden rounded-lg p-2 text-(--color-text-subtle) transition hover:bg-(--color-border) md:inline-flex"
