...
export async function GET(request: NextRequest) {
  try {
    ...
    // Get schedules (including shifts) for employees on the date
    ...
    const scheduleMap = new Map<string, typeof schedules[number]>(
      schedules.map((s) => [s.employeeId, s])
    );

    // Build result with attendance status and shift
    const results = employees.map((emp: EmployeeItem) => {
      ...
      return {
        ...
        status: attendance?.status || (schedule?.isHoliday ? "HOLIDAY" : "ABSENT"),
        clockIn: attendance?.clockIn || null,
        clockOut: attendance?.clockOut || null,
        workMode: attendance?.workMode || schedule?.workMode || null,
