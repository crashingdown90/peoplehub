...
type ShiftWithScheduleCount = Prisma.ShiftGetPayload<Record<string, never>> & {
  _count?: { schedules: number; defaultShiftEmployees: number };
};
  ...
  static async getShifts(
    ...
    const [shifts, total] = await Promise.all([
      prisma.shift.findMany({
        where,
        include: { _count: { select: { schedules: true, defaultShiftEmployees: true } } },
