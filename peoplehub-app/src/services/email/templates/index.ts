...
export function payslipDetail(vars: Record<string, string>): EmailTemplate {
  const content = `
    <h2>Slip Gaji - {{period}}</h2>
    <p>Halo <strong>{{employeeName}}</strong>,</p>
    <p>Berikut ringkasan slip gaji Anda:</p>

    <div class="info-box">
      <table>
        <tr><th>Periode</th><td>{{period}}</td></tr>
        <tr><th>Gaji Kotor</th><td>{{grossSalary}}</td></tr>
        <tr><th>Total Potongan</th><td>{{totalDeductions}}</td></tr>
        <tr><th>Gaji Bersih</th><td style="font-size: 18px; font-weight: bold; color: #22c55e;">{{netSalary}}</td></tr>
      </table>
    </div>
  ...
  return {
    name: "payslipDetail",
    subject: "Slip Gaji {{period}}",
    html: baseTemplate(content, "Slip Gaji"),
    text: `Slip gaji {{period}}. Gaji bersih: {{netSalary}}. Lihat detail di {{payslipUrl}}.`,
  };
}
