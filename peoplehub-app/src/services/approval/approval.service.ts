...
export class ApprovalService {
  ...
  /**
   * Get approval statistics (role-aware: counts only what each role can actually see)
   */
  static async getStats(context: RequestContext): Promise<ServiceResponse<ApprovalStats>> {
    ...
    // Determine which status each role should see per type
    // MANAGER sees PENDING (first-level approval)
    // HRD sees APPROVED_MANAGER (second-level) for leave, PENDING for corrections
    // FINANCE sees APPROVED_MANAGER (second-level) for travel & reimburse
    // SUPER_ADMIN sees PENDING (all)

    // --- LEAVE ---
    if (["MANAGER", "HRD", "SUPER_ADMIN"].includes(context.role)) {
      let leaveWhere: Record<string, unknown> = { tenantId: context.tenantId };
      if (context.role === "MANAGER") {
        leaveWhere.status = "PENDING";
      } else if (context.role === "HRD") {
        leaveWhere.status = { in: ["PENDING", "APPROVED_MANAGER"] };
      } else {
        leaveWhere.status = "PENDING";
      }
      if (context.role === "MANAGER" && context.employeeId) {
        ...
        leaveWhere = { ...leaveWhere, employeeId: { in: subordinates.map(s => s.id) } };
      }

      const leavePending = await prisma.leaveRequest.count({ where: leaveWhere });
    ...
    if (["MANAGER", "HRD", "SUPER_ADMIN"].includes(context.role)) {
      let correctionWhere: Record<string, unknown> = { tenantId: context.tenantId, status: "PENDING" as ApprovalStatus };
      if (context.role === "MANAGER" && context.employeeId) {
        const subordinates = await prisma.employee.findMany({
          where: withTenant(context, { managerId: context.employeeId }),
          select: { id: true },
        });
        correctionWhere = { ...correctionWhere, employeeId: { in: subordinates.map(s => s.id) } };
      }

      const correctionPending = await prisma.attendanceCorrection.count({ where: correctionWhere });
    ...
    // --- TRAVEL ---
    if (["MANAGER", "FINANCE", "SUPER_ADMIN"].includes(context.role)) {
      let travelWhere: Record<string, unknown> = { tenantId: context.tenantId };
      if (context.role === "MANAGER") {
        travelWhere.status = "PENDING";
      } else if (context.role === "FINANCE") {
        travelWhere.status = { in: ["PENDING", "APPROVED_MANAGER"] };
      } else {
        travelWhere.status = "PENDING";
      }
      if (context.role === "MANAGER" && context.employeeId) {
        ...
        travelWhere = { ...travelWhere, employeeId: { in: subordinates.map(s => s.id) } };
      }

      const travelPending = await prisma.travelRequest.count({ where: travelWhere });
    ...
    // --- REIMBURSE ---
    if (["MANAGER", "FINANCE", "SUPER_ADMIN"].includes(context.role)) {
      let reimburseWhere: Record<string, unknown> = { tenantId: context.tenantId };
      if (context.role === "MANAGER") {
        reimburseWhere.status = "PENDING";
      } else if (context.role === "FINANCE") {
        reimburseWhere.status = { in: ["PENDING", "APPROVED_MANAGER"] };
      } else {
        reimburseWhere.status = "PENDING";
      }
      if (context.role === "MANAGER" && context.employeeId) {
        ...
        reimburseWhere = { ...reimburseWhere, employeeId: { in: subordinates.map(s => s.id) } };
      }

      const reimbursePending = await prisma.reimburseRequest.count({ where: reimburseWhere });
      stats.pending += reimbursePending;
      stats.byType.REIMBURSE = reimbursePending;
    }

    // Count approved/rejected totals
    const [leaveApproved, leaveRejected] = await Promise.all([
      prisma.leaveRequest.count({ where: { tenantId: context.tenantId, status: "APPROVED" as ApprovalStatus } }),
      prisma.leaveRequest.count({ where: { tenantId: context.tenantId, status: "REJECTED" as ApprovalStatus } }),
    ]);
  ...
  private static async getLeaveApprovals(
    ...
    // Manager approval - PENDING status for subordinates only
    if (context.role === "MANAGER") {
      where.employeeId = { in: subordinateIds };
      where.status = "PENDING";
    }
    // HRD approval - sees PENDING and APPROVED_MANAGER (handles cases with/without manager)
    else if (context.role === "HRD") {
      where.status = { in: ["PENDING", "APPROVED_MANAGER"] };
    }
